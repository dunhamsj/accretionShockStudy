...
for m in range( len( M ) ):
    ...
    for rs in range( len( Rs[0] ) ):
        ...
        ID_NR = 'NR' + ID
        timeGR, dataGR \
          = np.loadtxt( dataDirectory + 'LatFlux_{:}.dat'.format( ID_GR ) )
        timeNR, dataNR \
          = np.loadtxt( dataDirectory + 'LatFlux_{:}.dat'.format( ID_NR ) )

        f = open( dataDirectory + '{:}_LegendrePowerSpectrum.dat' \
                  .format( ID_NR ) )
        s = f.readline()
        ...
        RsFileNameGR \
          = dataDirectory + '{:}_ShockRadiusVsTime.dat'.format( ID_GR )
        TimeGR, RsAveGR, RsMinGR, RsMaxGR = np.loadtxt( RsFileNameGR )
        RsFileNameNR \
          = dataDirectory + '{:}_ShockRadiusVsTime.dat'.format( ID_NR )
